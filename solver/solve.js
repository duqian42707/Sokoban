...
const solve = (xsbText) => {
    return new Promise(resolve => {
        const data = xsbToBlocks(xsbText);
        const TREE = 1,
            EMPTY = 0;
        ...
        // });
        data.filter(t => t.type === BlockType.WALL).forEach(t => {
            map[(t['row'] + 1) * BOUND_X + (t['col'] + 1)] = TREE;
        })

        // const allBuckets = new Set(data.buckets.map(b => b[1] * BOUND_X + b[0]));
        const allBuckets = new Set(data.filter(b => b.type === BlockType.BOX || b.type === BlockType.BOX_ON_GOAL).map(b => (b['row'] + 1) * BOUND_X + (b['col'] + 1)));

        // const spots = data.spots.map(s => s[1] * BOUND_X + s[0]);
        const spots = data.filter(b => b.type === BlockType.GOAL || b.type === BlockType.BOX_ON_GOAL || b.type === BlockType.MAN_ON_GOAL)
            .map(s => (s['row'] + 1) * BOUND_X + (s['col'] + 1));
